...
namespace MOP.Helpers
{
    static class SaveManager
    {
        ...
        public static bool IsSatsumaLoadedCompletely()
        {
            try
            {
                GameObject satsuma = GameObject.Find("CORRIS");
                if (satsuma == null)
                {
                    return false;
                }

                GameObject cylinderHead = Resources.FindObjectsOfTypeAll<GameObject>().FirstOrDefault(g => g.name == "cylinder head(Clone)");
                if (cylinderHead == null)
                {
                    return true;
                }

                GameObject block = Resources.FindObjectsOfTypeAll<GameObject>().FirstOrDefault(g => g.name == "block(Clone)");
                if (block == null)
                {
                    return true;
                }
            ...
            {
                GameObject corris = GameObject.Find("CORRIS");
                if (corris == null)
                {
                    return false;
                }
                ExceptionManager.New(ex, false, "Satsuma loading check encountered an exception, bypassing details.");
                return true;
            }
        }
        ...
        public static bool IsCarAssembledWithMSCEditor()
        {
            ...
            try
            {
                ...
                GameObject bodyDb = GameObject.Find("Database/DatabaseBody");
                GameObject mechDb = GameObject.Find("Database/DatabaseMechanics");
                GameObject motorDb = GameObject.Find("Database/DatabaseMotor");
                
                if (bodyDb == null || mechDb == null || motorDb == null)
                {
                    return false;
                }
                
                CountBoltedAndInstalled(bodyDb, ref boltedCount, ref installedCount);
                CountBoltedAndInstalled(mechDb, ref boltedCount, ref installedCount);
                CountBoltedAndInstalled(motorDb, ref boltedCount, ref installedCount);
            ...
            {
                ExceptionManager.New(ex, false, "MSCEditor check failed, skipping.");
                return false;
            }
        }
