    ...
    class ItemBehaviour : MonoBehaviour, ILod
    {
        ...
        void ToggleActive(bool enabled)
        {
            try
            {
                ...
                if (gameObject.activeSelf == enabled || transform.root.gameObject.name == "CORRIS")
                {
                    return;
                }
        ...
        void TogglePhysicsOnly(bool enabled)
        {
            try
            {
                ...
                if (!gameObject.name.EqualsAny("lottery ticket(xxxxx)", "envelope(xxxxx)", "teimo advert pile(itemx)"))
                {
                    if (enabled)
                    {
                        transform.position = position;
        ...
        void FsmFixes()
        {
            ...
            if (fsm != null)
            {
                foreach (var st in fsm.FsmStates)
                {
                    ...
                    {
                        case "Destroy self":
                            this.gameObject.FsmInject("Destroy self", RemoveSelf);
                            break;
                        case "Destroy":
                            this.gameObject.FsmInject("Destroy", RemoveSelf);
                            break;
                        case "Destroy 2":
                            this.gameObject.FsmInject("Destroy 2", RemoveSelf);
                            break;
                    }
                }
            }
            ...
            {
                gameObject.FsmInject("Is garbage", RemoveSelf);
