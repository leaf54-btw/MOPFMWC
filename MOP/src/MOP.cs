...
namespace MOP
{
    ...
    {
        public override string ID => "MOPFMWC";
        public override string Name => "MODERN OPTIMIZATION PLUGIN MY WINTER CAR";
        public override string Author => "Athlon, leaf54"; //Your Username
        ...
        public override string Description => "The <color=yellow>ultimate</color> My Winter Car optimization project!";
...
#endif
        {
            ...
            UpdateSettingsUI();
#else
            Settings.AddButton("FAQ", () => ShowDialog(FaqLink));
            Settings.AddButton("WIKI", () => ShowDialog(WikiLink));
            Settings.AddButton("HOMEPAGE", () => ShowDialog(HomepageLink));
            Settings.AddButton("GITHUB", () => ShowDialog(GitHubLink));
            Settings.AddButton("NEXUSMODS", () => ShowDialog(NexusModsLink));
            Settings.AddButton("DONATE", OnDonateButtonClick, new Color32(37, 59, 128, 255), new Color(1, 1, 1));

            // Activating objects.
            Settings.AddHeader("DESPAWNING");
            ActiveDistance = Settings.AddSlider("activateDistanceNew", "ACTIVATE DISTANCE", 0, 4, 2, textValues: activeDistanceText);
            Settings.AddText("PERFORMANCE MODE");
            ModePerformance = Settings.AddCheckBoxGroup("modePerformance", "PERFORMANCE", false, "performanceMode", MopSettings.UpdatePerformanceMode);
            ModeBalanced = Settings.AddCheckBoxGroup("modeBalanced", "BALANCED", true, "performanceMode", MopSettings.UpdatePerformanceMode);
            ModeQuality = Settings.AddCheckBoxGroup("modeQuality", "QUALITY", false, "performanceMode", MopSettings.UpdatePerformanceMode);
            ModeSafe = Settings.AddCheckBoxGroup("modeSafe", "<color=red>SAFE</color>", false, "performanceMode", MopSettings.UpdatePerformanceMode);
            modeWarningText = Settings.AddText("");

            // Graphics
            Settings.AddHeader("GRAPHICS");
            LimitFramerate = Settings.AddCheckBox("limitFramerate", "LIMIT FRAMERATE", false, MopSettings.UpdateFramerateLimiter);
            FramerateLimiter = Settings.AddSlider("framerateLimiterUpdated", "FRAMERATE LIMITER (FPS)", 20, 200, 60, MopSettings.UpdateFramerateLimiter);
            ShadowDistance = Settings.AddSlider("shadowDistance", "SHADOW DISTANCE (METERS, 200 DEFAULT)", 0, 2000, 200, MopSettings.UpdateShadows);
            KeepRunningInBackground = Settings.AddCheckBox("keepRunningInBackground", "RUN GAME IN BACKGROUND", true, MopSettings.ToggleBackgroundRunning);
            Settings.AddText("If unchecked, the game will be paused when the game's window looses focus.");
            DynamicDrawDistance = Settings.AddCheckBox("dynamicDrawDistance", "DYNAMIC DRAW DISTANCE", true);
            Settings.AddText("MOP will adjust the draw distance, according to the current situation\n(ex. lower it while inside of a building).");

            GameResolution = Settings.AddDropDownList("", "RESOLUTION", resolutions.ToArray(), selected, () =>
            {
            ...
            // Rules
            Settings.AddHeader("RULES");
            Settings.AddButton("LEARN MORE", () => ShowDialog(RuleFilesWikiLink));
            RulesAutoUpdate = Settings.AddCheckBox("rulesAutoUpdate", "UPDATE RULES AUTOMATICALLY", true, WarningDisableAutoUpdate);
            RulesAutoUpdateFrequency = Settings.AddSlider("ruleAutoUpdateFrequendy", "AUTO-UPDATE FREQUENCY", 0, 3, 2, textValues: rulesAutoUpdateFrequencyText);
            VerifyRuleFiles = Settings.AddCheckBox("verifyRuleFiles", "VERIFY RULE FILES", true);
            DeleteUnusedRules = Settings.AddCheckBox("deleteUnusedRules", "AUTOMATICALLY DELETE UNUSED RULES", false);
            Settings.AddButton("DELETE UNUSED RULES", RulesManager.DeleteUnused);
            Settings.AddButton("FORCE UPDATE", ForceRuleFilesUpdate);

            // Other
            Settings.AddHeader("OTHER");
            DestroyEmptyBottles = Settings.AddCheckBox("destroyEmptyBottles", "DESTROY EMPTY BOTTLES", false);
            Settings.AddText("Empty bottles, created by player drinking, won't spawn.");
            DisableEmptyItems = Settings.AddCheckBox("disableEmptyItems", "DISABLE EMPTY ITEMS", false);
            Settings.AddText("Items, which status changes to 'EMPTY', will be automatically disabled, in order to save on PC resources.");
            AlwaysDisableSkidmarks = Settings.AddCheckBox("alwaysDisableSkidmarks", "DISABLE SKIDMARKS", false);
            Settings.AddText("Skidmarks cause a massive memory leak, every time you brake/accelerate hard. Disabling them mitigates that problem.");

            // Logging
            Settings.AddHeader("LOGGING");
            Settings.AddButton("OPEN LOGS FOLDER", ExceptionManager.OpenCurrentSessionLogFolder);
            Settings.AddButton("GENERATE MOD REPORT", ExceptionManager.GenerateReport);
            Settings.AddButton("DELETE ALL LOGS", ExceptionManager.DeleteAllLogs);

            // Supporters
            Settings.AddHeader("SUPPORTERS");
            Settings.AddText("<color=yellow>" + GetSupporters() + "</color>" +
                "\n\nDo you want to see your name here? Send a donate!");

            // Changelog
            Settings.AddHeader("CHANGELOG");
            Settings.AddText(GetChangelog(false));

            // Info
            Settings.AddHeader("INFO");
            Settings.AddText(GetFooter());
#endif
        }
        #endregion
