...
namespace MOP.Vehicles.Cases
{
    internal class Hayosiko : Vehicle
    {
        ...
        public Hayosiko(string gameObjectName = "HAYOSIKO(1500kg, 250)") : base(gameObjectName)
        {
            ...
            Transform odometer = transform.Find("Odometer");
            if (odometer != null)
            {
                PlayMakerFSM fsm = odometer.gameObject.GetComponent<PlayMakerFSM>();
        ...
        {
            if (partialDisableItems == null || partialDisableItems.Length == 0)
            {
                return;
            }
