...
namespace MOP.Places
{
    class Yard : Place
    {
        ...
        public Yard() : base("YARD")
        {
            ...
            Transform uncle = transform.Find("UNCLE");
            if (uncle != null)
            {
                PlayMakerFSM fsm = uncle.GetComponent<PlayMakerFSM>();
                if (fsm != null) fsm.Fsm.RestartOnEnable = false;
            }
            ...
            try
            {
                ...
                if (fridgePoint)
                {
                    PlayMakerFSM fsm = fridgePoint.GetPlayMaker("Chilling");
                    if (fsm != null)
                    {
                        fridgeRunning = fsm.FsmVariables.GetFsmBool("Kitchen");
                    }
                }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                if (sauna != null)
                {
                    ...
                    {
                        PlayMakerFSM fsm = saumaSimulation.GetPlayMaker("Time");
                        if (fsm != null)
                        {
                            saunaStoveHeat = fsm.FsmVariables.GetFsmFloat("StoveHeat");
                        }
                    }
                }
            }
        ...
        private static void FixGarageDoors()
        {
            ...
            if (garageDoors)
            {
                Transform left = garageDoors.transform.Find("DoorLeft/Coll");
                if (left != null)
                {
                    PlayMakerFSM fsm = left.GetComponent<PlayMakerFSM>();
                    if (fsm != null) fsm.Fsm.RestartOnEnable = false;
                }

                Transform right = garageDoors.transform.Find("DoorRight/Coll");
                if (right != null)
                {
                    PlayMakerFSM fsm = right.GetComponent<PlayMakerFSM>();
                    if (fsm != null) fsm.Fsm.RestartOnEnable = false;
                }
            }
        }
        ...
        {
            Transform saunaTrans = transform.Find("Building/SAUNA");
            if (saunaTrans != null)
            {
                string[] paths = {
                    "Sauna/Kiuas/ButtonPower",
                    "Sauna/Kiuas/ButtonTime",
                    "Sauna/Kiuas/StoveTrigger",
                    "Sauna/Simulation"
                };
                foreach (string p in paths)
                {
                    Transform t = saunaTrans.Find(p);
                    if (t != null)
                    {
                        PlayMakerFSM fsm = t.GetComponent<PlayMakerFSM>();
                        if (fsm != null) fsm.Fsm.RestartOnEnable = false;
                    }
                }
            }
