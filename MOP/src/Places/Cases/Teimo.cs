...
namespace MOP.Places
{
    class Teimo : Place
    {
        ...
        private void AdvertFix()
        {
            ...
            Transform spawn = transform.Find("AdvertSpawn");
            if (spawn != null)
            {
                spawn.parent = null;
            }
        ...
        private void InjectVideoPoker()
        {
            ...
            if (videoPoker != null && videoPoker.gameObject != null)
        ...
        private void FixSlotAndPoker()
        {
            ...
            Transform videoPokerMesh = transform.Find("LOD/VideoPoker/Hatch/Pivot/mesh");
            if (videoPokerMesh != null && DisableableChilds != null)
            {
                DisableableChilds.Remove(videoPokerMesh);
            }

            // Fix for Z-fighting of slot machine glass.
            Transform slotGlass = transform.Find("LOD/GFX_Store/SlotMachine/slot_machine 1/slot_machine_glass");
            if (slotGlass != null)
            {
                var rend = slotGlass.gameObject.GetComponent<Renderer>();
                if (rend != null && rend.material != null) rend.material.renderQueue = 3001;
            }
        }

        private void TeimoShitFix()
        {
            Transform teimoShop = transform.Find("TeimoInShop");
            if (teimoShop != null)
            {
                PlayMakers.AddRange(teimoShop.GetComponents<PlayMakerFSM>());
            }
            ...
            {
                foreach (var t in teimoShit)
                {
                    if (t != null) DisableableChilds.Add(t);
                }
            }
        }
        ...
        private static void FixWindowBreaking()
        {
            ...
            if (store != null)
            {
                Transform storeBreakableWindow = store.transform.Find("LOD/GFX_Store/BreakableWindows/BreakableWindow");
                ...
                Transform storeBreakableWindowPub = store.transform.Find("LOD/GFX_Pub/BreakableWindowsPub/BreakableWindowPub");
        ...
        private void CheckMicrowaveDistanceFix()
        {
            ...
            if (inventory != null)
            {
                var fsm = inventory.gameObject.GetComponent<PlayMakerFSM>();
                if (fsm != null)
                {
                    var state = fsm.GetState("Check distance");
                    if (state != null && state.Actions != null && state.Actions.Length > 1)
                    {
                        FloatCompare checkDistance = state.Actions[1] as FloatCompare;
        ...
        private void UncleRagdollFix()
        {
            ...
            var lod = transform.Find("LOD");
            if (lod != null)
            {
                transform.Find("RagDoll")?.SetParent(lod);
            }
        }
        ...
        private void GrillBoxFix()
        {
            ...
            Transform grillbox = transform.Find("LOD/GFX_Pub/Microwave/GrillboxMicro");
            if (grillbox != null)
            {
                if (grillbox.gameObject.GetComponent<SausageInMicrowaveBehaviour>() == null)
                {
                    grillbox.gameObject.AddComponent<SausageInMicrowaveBehaviour>();
                }
            }
        }
        ...
        {
            GameObject pokerObj = GameObject.Find("VideoPoker");
            if (pokerObj != null)
            {
                Transform poker = pokerObj.transform;
                if (DisableableChilds != null)
                {
