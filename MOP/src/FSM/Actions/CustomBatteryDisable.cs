...
namespace MOP.FSM.Actions
{
    ...
    public class CustomBatteryDisable : FsmStateAction
    {
        ...
        {
            var dbWiring = GameObject.Find("Database/DatabaseWiring/WiringBatteryMinus");
            if (dbWiring != null)
            {
                var pm = dbWiring.GetPlayMaker("Data");
                if (pm != null)
                {
                    fsmBoolInstalled = pm.FsmVariables.FindFsmBool("Installed");
                }
            }
            var corris = GameObject.Find("CORRIS");
            if (corris != null)
            {
                var terminal = corris.transform.Find("Wiring/Parts/battery_terminal_minus(xxxxx)");
                if (terminal != null)
                {
                    batteryTerminalMinus = terminal.gameObject;
                }
            }
        }
        ...
        { 
            if (batteryTerminalMinus != null && fsmBoolInstalled != null)
            {
